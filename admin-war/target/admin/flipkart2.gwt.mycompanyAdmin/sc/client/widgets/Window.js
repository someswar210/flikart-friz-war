...
isc.Window.addProperties({
	...
	useBackMask: isc.Browser.isIE && isc.Browser.minorVersion >= 5.5,
    ...
    //> @attr window.modalMask (AutoChild : null : R)
    ...
    //> @attr window.body (AutoChild : null : R)
    ...
    //> @attr window.header (AutoChild : null : R)
    ...
    //>	@attr	window.showHeader		(boolean : true : IRWA)
    ...
    //> @attr window.headerBackground (AutoChild : null : R)
    ...
    //> @attr window.headerLabel (AutoChild : null : R)
    ...
    //>	@attr	window.title		(string : "Untitled Window" : [IRW])
    ...
    //> @attr window.headerIcon (AutoChild : null : R)
    ...
    //> @attr window.closeButton (AutoChild : null : R)
    ...
    //> @attr window.minimizeButton (AutoChild : null : R)
    ...
    //> @attr window.restoreButton (AutoChild : null : R)
    ...
    //> @attr window.maximizeButton (AutoChild : null : R)
    ...
    //> @attr window.statusBar (AutoChild : null : R)
    ...
    //> @attr window.resizer (AutoChild : null : R)
...
makeHeader : function () {
    ...
    var header = this.addAutoChild("header", {styleName:this.headerStyle});
...
headerLabel_autoMaker : function () {
    ...
    var headerLabelParent = isc.Canvas.create({
        autoDraw:false,
        _generated:true,
    ...
    var dragRepo = this.canDragReposition;
	// if the Window is moveable, make the header draggable
	if (dragRepo) {
		headerLabelParent.canDragReposition = true;
        headerLabelParent.dragTarget = this;
    ...
    headerLabelParent.addChild(headerLabel);
...
makeBody : function() {
	...
	if (this.src) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (isc.isAn.Object(items[i])) items[i].locatorParent = this;
        }
	}
...
_resetContentRestoreStats : function () {
    if (this.body) {
        this.body.scrollTo(null, this._bodyRestoreScrollTop);
    ...
    if (footer) {
        if (footerSize > 0) {
            if (footer.getHeight() != footerSize) {
                ...
                footer.scrollTo(null, scrollBottom - footer.getViewportHeight());
            }
    ...
    if (body) {
        if (bodySize > 0) {
            if (body.getHeight() != bodySize) {
                ...
                body.scrollTo(null, scrollBottom - body.getViewportHeight());
            }
...
closeClick : function () {
    ...
    // best to destroy() it.
	this.hide();
    return false; // cancel the mouse click
}
