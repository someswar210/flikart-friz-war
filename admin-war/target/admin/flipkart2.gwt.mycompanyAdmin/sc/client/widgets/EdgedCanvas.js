...
getInnerHTML : function () {
    ...
    //    (isc.Canvas._fixPNG() && extension.toLowerCase() == ".png") || isc.Browser.isMoz) {
        ...
        cellEnd = imgHTML.substring(dotIndex) + "</TD>";
    ...
    output.append(this._$edgeTableStart,
                  this._leftEdge, this._$edgeColMid, this._rightEdge, this._$rightAngle,
                  "<TR HEIGHT=", this._topEdge, this._$rightAngle);
                  
    this._writeEdgeCells(cellStart, this._topEdge, mediaStart, cellEnd, 0, 2, edges, output);

    if (isc.Browser.isIE && isc.Browser.isStrict) {
        // write explicit size for IE strict, which will be updated on resize
        output.append("</TR><TR HEIGHT=", 
    ...
    if (isc.Browser.isMoz) {
        // Moz needs explicit 100% on the central cells or the side rails revert to
        // 1px whenever the Canvas is moved.  Natively reproducible.
        this._writeEdgeCells(cellStart, "100%", mediaStart, cellEnd, 3, 5, edges, output);
    } else {
        this._writeEdgeCells(nonHeightStart, null, mediaStart, cellEnd, 3, 5, edges, output);
    }

    output.append("</TR><TR HEIGHT=", this._bottomEdge, ">");

    this._writeEdgeCells(cellStart, this._bottomEdge, mediaStart, cellEnd, 6, 8, edges, output);
...
_$classEquals:" class=",
_writeEdgeCells : function (cellStart, height, mediaStart, cellEnd, start, end, edges, output) {
    for (var i = start; i <= end; i++) {
        var edgeName = this._allEdges[i];
        ...
        if (edges[edgeName] || (this.showCenter && edgeName == isc.Canvas.CENTER)) {
            output.append(cellStart, height, 
                          classEquals, styleName,
                          mediaStart, this.getEdgePrefix(edgeName), 
                          isc._underscore, edgeName, cellEnd);
        } else {
