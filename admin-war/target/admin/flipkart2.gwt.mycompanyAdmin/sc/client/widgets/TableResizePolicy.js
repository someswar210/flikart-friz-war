...
applyTableResizePolicy : function (items, totalWidth, totalHeight, 
	...
	for (itemNum = 0; itemNum < items.length; itemNum++) {
        ...
        var isACanvas = isc.isA.Canvas(item),
            // Note - use getWidth() / getHeight() on canvii, to allow us to store height / width
            // with different (internal) property names if we choose to do so.
		    width = isACanvas ? item.getWidth() : item.width,
		...
		if (width == "*") {
			...
			var skipBefore = (item.showTitle && orientation == isc.Canvas.LEFT) ? 1 : 0,
				skipAfter = (item.showTitle && orientation == isc.Canvas.RIGHT) ? 1 : 0,
