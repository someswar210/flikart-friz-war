...
_xmlSerialize : function (name, object, prefix, context) {
    ...
    // NOTE: must be able to send null, which potentially has a distinct meaning to the
    // backend (eg nulling out a text field vs setting to empty string).  Null is encoded
    // distinctly from empty string by simply omitting the explicit typing to String.
	if (object == null) return isc.Comm._xmlValue(name, "");
